...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER main Running distance: {running_distance};
    TASK: Write a product description that is tailored into this customer's Age group and running distance. Use age group specific slang.;
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and running distance;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account running distance {running_distance} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ...";
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "running_distance", "content"],
    template=template,
)
...
def get_hobby():
    input_text = st.text_input(label="Customers main running_distance", key="hobby_input")
    return input_text
...
if content_input:
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, running_distance=hobby_input, content=content_input)
